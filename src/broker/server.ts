...
const EXTERNAL_ID = process.env.CLAWOPS_EXTERNAL_ID ?? "";
...
async function mint(plan: {
  ...
  const out = await sts.send(
    new AssumeRoleCommand({
      ...
      // The elevated role's trust policy requires this shared secret
      // (iam/elevated-trust-temp.json). Only the broker holds it, so holding
      // the role arn is not enough to assume it -- an agent that learned the
      // arn still cannot. Sent only when configured, so a trust policy
      // without the condition still works locally.
